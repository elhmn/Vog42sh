...
/*   Updated: 2014/03/26 13:33:41 by troussel         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
#include "ftsh.h"
#include "ftsh_env.h"
...
#define OCR O_CREAT | O_RDWR
#define OA O_APPEND
#define SA S_IROTH | S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP
#define SB S_IROTH | S_IWUSR | S_IWGRP | S_IRUSR | S_IRGRP

static int		openfile(char *file, int in, int appmode)
{
	if (appmode)
		return (open(file, OCR | OA, SA));
	else if (!in)
		return (open(file, OCR, SB));
	else if (in)
