...
/*   Updated: 2014/03/07 16:23:48 by mriclet          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

t_for		*alloc_new_tree(t_takl *lst, int opt, t_for *prev)
{
	t_for	*ret;
...
t_for		*pars_for(t_tokl *lst)
{
	...
	loop = 1;
	elem = alloc_new_tree(lst, 0, NULL);
	first = elem;
	prev_pipe_w = elem->wood->data->pipe_w;
	while (loop)
	{
		elem->nxt = alloc_new_tree(lst, 1, elem);
		prev_pipe_w = elem->wood->data->pipe_w;
